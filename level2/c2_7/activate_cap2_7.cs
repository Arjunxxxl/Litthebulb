...
public class activate_cap2_7 : MonoBehaviour {
	...
	#region Singleton
	public static activate_cap2_7 Instance;
	void Awake()
	{
		Instance = this;	
	}
	...
	void Update () {
		...
		if(result[1] && !cap1 && activateCapacior1 && cap3){
		...
		if(activateCapacior2 && ( !result[4] || !cap2)){StartCoroutine(cap_chargeTime("activateCapacior2"));}
		if(activateCapacior3 && ( (!result[1] && activateCapacior1) || (cap3 && !activateCapacior1) || !cap3)){StartCoroutine(cap_chargeTime("activateCapacior3"));}
		...
		if(result[2] && activateCapacior3 && activateCapacior4 && !cap4 && !cap3){lit_bulb1 = true;}else{lit_bulb1 = false;}
	...
	{	bool[] cap_status = {(activateCapacior1 && ( !result[0] || !cap1)),
							(activateCapacior2 && ( !result[4] || !cap2)),
							(activateCapacior3 && ( (!result[1] && activateCapacior1) || (cap3 && !activateCapacior1) || !cap3)),
