...
public class activate_cap3_4 : MonoBehaviour {
	...
	#region Singleton
	public static activate_cap3_4 Instance;
	void Awake()
	{
		Instance = this;	
	}
	...
	void Update () {
		...
		if(result[3] && cap1 && (result[5] || result[7])){
			activateCapacior1 = true;
		}
		if(result[4] && cap2 && (result[5] || result[6])){
		...
		if(activateCapacior1 && !cap1 && result[0] && result[8] && result[2])
		 {lit_bulb1 = true;}else{lit_bulb1 = false;}
		
		if(activateCapacior2 && !cap2 && result[1] && result[9] && result[10])
	...
	{	bool[] cap_status = {(!result[3] || !cap1 || (!result[5] && result[7])) && activateCapacior1,
							(!result[4] || !cap2 || (!result[5] && result[6])) && activateCapacior2
							};
