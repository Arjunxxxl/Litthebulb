...
public class activate_cap3_6 : MonoBehaviour {
...
public GameObject capacitor1, capacitor2;
	...
	#region Singleton
	public static activate_cap3_6 Instance;
	void Awake()
	{
		Instance = this;	
	}
	...
	void Update () {
		...
		if(activateCapacior1 && !cap1 && activateCapacior2 && !cap2 && result[3] && ((result[5] && !result[4]) || result[6]) )
	...
	{	bool[] cap_status = {((!result[1] || !cap1) && activateCapacior1),
							((!result[0] || !cap2 || !result[2]) && activateCapacior2)
							};
