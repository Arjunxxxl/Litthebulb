...
public class activate_cap3_3 : MonoBehaviour {
	...
	#region Singleton
	public static activate_cap3_3 Instance;
	void Awake()
	{
		Instance = this;	
	}
	...
	void Update () {
		...
		if(result[0] && cap1 && cap2 && result[2]){
		...
		if(!result[0] || ! result[2] || !cap1 || !cap2) {
		...
		if(activateCapacior1 && activateCapacior2 && !cap1 && !cap2) {lit_bulb1 = true;}else{lit_bulb1 = false;}
	...
	{	bool[] cap_status = {((!result[0] || ! result[2] || !cap1 || !cap2) && activateCapacior1),
							((!result[0] || ! result[2] || !cap1 || !cap2) && activateCapacior2 )
							};
